...
begin
  ...
  case rgpSelecionar.ItemIndex of
    0: vSQL := vSQL + ' WHERE ' + RetCampoCodigo(nmTab) + ' = :PCD';
    1: vSQL := vSQL + ' WHERE ' + RetCampoDesc(nmTab) + ' CONTAINING :PCD';
  end;
  case rgpOrdenar.ItemIndex of
    0: vSQL := vSQL + ' ORDER BY ' + RetCampoCodigo(nmTab);
    1: vSQL := vSQL + ' ORDER BY ' + RetCampoCodigo(nmTab) + ' DESC ';
  end;
